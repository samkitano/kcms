...
class Users
{
    ...
    {
        return Tag::span(['class' => 'h-6'], __t('auth.'.$user->role ?? 'none'));
    }
    ...
    public static function userProfile($user): string
    {
        ...
        $roleSvgTitle = Tag::title(__t('auth.role'));
        $emailSvgTitle = Tag::title(__t('auth.email'));
        $name = Tag::div(['class' => 'text-black font-bold text-xl mb-2'], $user->name);
        $registeredLabel = Tag::strong(__t('auth.registered').': ');
        ...
        $lastActiveLabel = Tag::strong(__t('auth.last_active').': ');
